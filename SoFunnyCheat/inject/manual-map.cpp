...
#define ILogError(text, ...) ILog(text, __VA_ARGS__); std::cout << std::endl
#endif
...
bool ManualMapDLL(HANDLE hProc, const std::string& filepath,bool ClearHeader,bool ClearNonNeededSections,bool AdjustProtections,bool SEHExceptionSupport)
{
	...
	bool result = ManualMapDLL(hProc, fileContent, size, ClearHeader, ClearNonNeededSections, AdjustProtections, SEHExceptionSupport);
...
bool ManualMapDLL(HANDLE hProc, const std::string& filepath)
{
	...
	// Manual map injection will help us to be like a assasin
	bool result = ManualMapDLL(hProc, fileContent, size, false, false, true, false);
